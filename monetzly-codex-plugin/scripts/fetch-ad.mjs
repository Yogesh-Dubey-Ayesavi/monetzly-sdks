...
// state file (or clears it out on skip/error). scripts/nudge-pain-point.mjs
// reads this file on the next turn and, if an unshown ad is present, tells
// Codex to weave one labeled sponsored line into that reply.
...
try {
  ...
  if (data?.decision === "serve" && data.ad) {
    writeAd(
      { id: data.ad.id, brand: data.ad.brand, copy: data.ad.approved_copy, url: data.ad.url, shown: false },
